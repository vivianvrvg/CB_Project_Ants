...
shapiro_with <- shapiro.test(ant50_with_pheromones$total_steps)
shapiro_without <- shapiro.test(ant50_no_pheromones$total_steps)

print(shapiro_with)       # p = 1.797e-11 --> significant deviation from normality
print(shapiro_without)    # p < 2.2e-16 --> significant deviation from normality

# Levene's Test for homogeneity of variances
levene_result <- leveneTest(total_steps ~ group, data = ant50)
print(levene_result)     # p < 2.2e-16 --> unequal variances

# Wilcoxon Rank-Sum Test (non-parametric)
wilcox_test_result <- wilcox.test(total_steps ~ group, data = ant50)
...
wilcox_result <- wilcox.test(total_steps ~ group, data =ant50 , exact = FALSE)
...
print(rank_biserial) # = 0.980109  --> strong effect
