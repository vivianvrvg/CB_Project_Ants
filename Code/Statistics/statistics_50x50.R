...
shapiro_with_grid50 <- shapiro.test(grid50_with_pheromones$total_steps)
shapiro_without_grid50 <- shapiro.test(grid50_no_pheromones$total_steps)

print(shapiro_with_grid50)       # p < 2.2e-16 --> significant deviation from normality
print(shapiro_without_grid50)    # p < 2.2e-16 --> significant deviation from normality

# Levene's Test for homogeneity of variances
levene_result_grid50 <- leveneTest(total_steps ~ group, data = grid50)
print(levene_result_grid50)     # p = 0.01421 --> unequal variances

# Wilcoxon Rank-Sum Test (non-parametric)
wilcox_test_result_grid50 <- wilcox.test(total_steps ~ group, data = grid50)
print(wilcox_test_result_grid50)   # p-value < 2.2e-16 --> significant difference between groups

# Extract Z statistic (Z is derived from the p-value and W statistic)
Z <- qnorm(wilcox_test_result_grid50$p.value / 2, lower.tail = FALSE)  # Two-tailed test
Z 

# Total sample size
n1 <- sum(grid50$group == "with_pheromones")
n2 <- sum(grid50$group == "no_pheromones")
N <- n1 + n2

# Compute r
r <- Z / sqrt(N)
print(r)  # Standardized effect size: 0.2491409
